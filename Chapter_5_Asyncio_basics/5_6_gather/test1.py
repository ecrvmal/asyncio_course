...
async def sensor_data(sensor_name):
    result = space_data(sensor_name)
    await asyncio.sleep(space_time(sensor_name))
    result = f"{sensor_name}_data: {''.join(result)}"
    return result

async def main():
    sensors.sort()
    print(sensors)
    task_list = [sensor_data(sensor) for sensor in sensors]
    print(task_list)
    result = list(await asyncio.gather(*task_list))
    header = f"Результаты проведения теста типа {test_type}:"
