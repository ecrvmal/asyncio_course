...
async def withdraw_money(amount):                     # Асинхронная функция для снятия денег
...
async def main():                                     # Главная асинхронная функция
    task1 = asyncio.create_task(withdraw_money(900))  # Создание задачи для асинхронного снятия средств.
                                                      # Используется asyncio.create_task для создания и запуска корутины без блокировки основного потока
    task2 = asyncio.create_task(withdraw_money(900))  # То же самое для второй операции снятия денег
