...
async def main():
    tasks = [asyncio.create_task(call_company(comp_info)) for comp_info in data]
    # await asyncio.gather(*tasks, return_exceptions=True)
    # await asyncio.sleep(5.5)
    done, pending = await asyncio.wait(tasks, timeout=5.5)
    ...
    for el in pending:
        print(f'task {el.get_name()} cancelled')
        el.cancel()
