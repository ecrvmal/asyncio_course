...
banned_words = ["bug", "error", "exception", "fail", "crash", "hang", "slow",
...
async def check_message(input_message):
    ...
    role = the_task.get_name()
    match role:
        case 'admin':
            return f'admin: {input_message}'
        case 'moderator':
            for word in banned_words:
                if word in input_message:
                    input_message = input_message.replace(word, '****')
            return f'moderator: {input_message}'
        case 'student':
            for word in banned_words:
                if word in input_message:
                    return f'student: В сообщении есть запрещённое слово, сообщение скрыто'
            return f'student: {input_message}'
        case 'black_list_user':
            return f'black_list_user: Пользователь забанен, сообщение скрыто'
    return f'None: ERROR_USER_NONE'
...
async def main():
    for el in messages:
        ...
        role = el['role']
        try:
            the_task = asyncio.create_task(check_message(text), name=role)
            await the_task
            print(f'{the_task.result()}')
