...
async def robot_reaction(event, bot, message):
    await event.wait()
    await asyncio.create_task(speech_synt(bot, message))
...
async def _event(id, id_sm, event):
    if id == id_sm:
        ...
        print(f'Спокойно, ждем сержанта!')
...
async def birthday():
    ...
    message = f"Повелитель механизмов! Долгих лет! Ты ведешь нас! Слава сержанту! Ура!"
    # Создаем событие
    ...
    await asyncio.create_task(sensor_id_124(_event, id_sm, happy_event))
